...
static void test_mma_fa2_flash() {
    std::printf("\n[1h] M10.7 mma.sync FA-2-shape flash (STAGES=3 + K-via-ldmatrix), multi-N verify (FP16 in / FP32 out)\n");
    for (int N : {128, 1024, 2048}) {
        std::srand(7 + N);
        std::vector<float>  h_Q(N * D), h_K(N * D), h_V(N * D);
        std::vector<__half> h_Qh(N * D), h_Kh(N * D), h_Vh(N * D);
        ...
        launch_flash_mma_fa2(d_Q, d_K, d_V, d_O, N);
        CUDA_CHECK_LAST();

        std::vector<float> h_got(N * D);
        CUDA_CHECK(cudaMemcpy(h_got.data(), d_O, N * D * sizeof(float), cudaMemcpyDeviceToHost));
        char name[64];
        std::snprintf(name, sizeof(name), "    N=%-5d 10.7 mma FA-2-shape", N);
        check(h_got, h_ref, name, /*rel*/2e-2f, /*abs*/5e-3f);
